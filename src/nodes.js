...
export const ProductNode = (imageArgs, reviews) =>
  createNodeFactory(PRODUCT, async node => {
    ...
    if (node.images && node.images.edges)
      node.images = await map(node.images.edges, async edge => {
        edge.node.localFile___NODE =
          process.env.NODE_ENV !== "development"
            ? await downloadImageAndCreateFileNode(
                {
                  id: edge.node.id,
                  url:
                    edge.node.originalSrc && edge.node.originalSrc.split(`?`)[0]
                },
                imageArgs
              )
            : undefined
          ...
          ? await downloadImageAndCreateFileNode(
              {
                id: node.image.id,
                url:
                  node.image.originalSrc && node.image.originalSrc.split(`?`)[0]
              },
              imageArgs
            )
          : undefined
